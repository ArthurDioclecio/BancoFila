...
update funcionario from  dependente d right not join funcionario.cod_func, d.cod_func set salario = salario*1.1;

/* 1 */
UPDATE funcionario f LEFT JOIN dependente d ON f.cod_func = d.cod_func SET f.sal_func = f.sal_func * 1.1;

/* 2 */
UPDATE funcionario f INNER JOIN dependente d ON f.cod_func = d.cod_func SET f.sal_func = f.sal_func * 1.1;


/* 3 */
UPDATE cliente c RIGHT JOIN conjuge d ON c.cod_cli = d.cod_cli SET c.renda_cli=c.renda_cli+100;


/* 4 */
UPDATE cliente c RIGHT JOIN conjuge d ON c.cod_cli = d.cod_cli SET c.renda_cli=c.renda_cli+400 where c.sexo_cli = 'f';

/* 5 */
UPDATE cliente c INNER JOIN pedido d ON c.cod_cli = d.cod_cli SET c.renda_cli=c.renda_cli+200;

/* 6 */
UPDATE pedido p INNER JOIN titulo_pedido t ON t.cod_tit='1' SET p.val_ped=p.val_ped*0.9;

/* 7 */
UPDATE titulo t LEFT JOIN titulo_pedido tp ON t.cod_tit = tp.cod_tit SET t.val_cd = t.val_cd * 0.9 WHERE tp.cod_tit IS NULL;
